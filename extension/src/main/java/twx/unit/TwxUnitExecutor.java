...
public class TwxUnitExecutor extends RemoteThing {
            ...
            try {
                ...
                if (te.getState().equals(ExecutionState.Scheduled)) {
                    ...
                    try {
                        ...
                        try {
                            executeOther(suite, "BeforeEach");
                        } catch(Throwable t) {
                            t.printStackTrace();
                            te.abort(false, "Error while executing BeforeEach for test suite " + suite + ": " + t.getMessage());
                        }                        
                        te.execute();
                        updateProperties();
                        
                        try {
                            executeOther(suite, "AfterEach");
                        } catch(Throwable t) {
                            t.printStackTrace();
                            te.abort(false, "Error while executing AfterEach for test suite " + suite + ": " + t.getMessage());
                        }                        
    ...
    private void flattenPlanRecursive(TestSuite plan, List<TestDefinition> list, String prefix) {
        for (TestDefinition td: plan.getTestCases()) {
            td.setId( prefix + td.getTestCase() ); 
        }
        list.addAll(plan.getTestCases());
        for (TestSuite child: plan.getSuites()) {
            flattenPlanRecursive(child, list, prefix + child.getName() + " > ");
        }
    }
    ...
    private List<TestDefinition> getFlatExecutionPlan(String testSuite, String defaultRunAs) throws TestingException {
        ...
        flattenPlanRecursive(plan, list, testSuite + " > ");
        return list;
    }
